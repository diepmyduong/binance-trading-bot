...
export const onCanceledRegisService = new Subject<IRegisService>();
// Gui mess cho nguoi do
onCanceledRegisService.subscribe(async (regisService) => {
    const { registerId, sellerId, basePrice, code } = regisService;
    ...
    if (pageAccount) {
        SettingHelper.load(SettingKey.REGIS_SERVICE_CANCELED_MSG).then((msg) => {
