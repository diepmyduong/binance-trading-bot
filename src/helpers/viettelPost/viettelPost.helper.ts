...
export class ViettelPostHelper {
  static host = configs.viettelPost.host;
  static getProvinces() {
    return Axios.get(`${this.host}/categories/listProvinceById?provinceId=`).then((res) =>
      get(res, "data.data")
    );
  }
  static getDistrict(proviceId: string) {
    return Axios.get(`${this.host}/categories/listDistrict?provinceId=${proviceId}`).then((res) =>
      get(res, "data.data")
    );
  }
  static getWard(districtId: string) {
    return Axios.get(`${this.host}/categories/listWards?districtId=${districtId}`).then(
      (res) =>
        get(res, "data.data", []).map((r: any) => ({
          wardId: r["WARDS_ID"].toString(),
          wardName: r["WARDS_NAME"],
          districtId: r["DISTRICT_ID"].toString(),
        })) as Ward[]
    );
  }
  static getPriceAll({
  ...
  static getListService() {
    return Axios.post(`${this.host}/categories/listService`, { TYPE: 2 }).then((res) =>
      get(res, "data.data")
    );
  }
  static getPricing({
  ...
  static updateOrder({ orderNumber, type, note, date }: UpdateOrderProps) {
    return Axios.post(
      `${this.host}/order/UpdateOrder`,
      {
        ...
        NOTE: note,
        DATE:
          type == UpdateOrderType.reorder ? moment(date).format("DD/MM/YYYY HH:mm:ss") : undefined,
      },
      {
        headers: { token: configs.viettelPost.token },
      }
  ...
  static trackingOrderStatus(orderNumber: string) {
    return Axios.get(`https://api.viettelpost.vn/api/setting/listOrderTrackingVTP`, {
      headers: { token: configs.viettelPost.token },
  ...
  static getLinkPrint(orderNumbers: string[]) {
    if (!configs.viettelPost.printToken)
      throw ErrorHelper.somethingWentWrong("Chưa cấu hình khoá in vận đơn từ Viettel Post.");
    return Axios.post(
      ...
      if (get(res, "data.status") == 200) return get(res, "data.message") as string;
      else throw ErrorHelper.externalRequestFailed(get(res, "data.message"));
    });
  }
}
