...
import { Button, ButtonProps } from "../../utilities/form/button";
import { Dropdown } from "../../utilities/popover/dropdown";

interface PropsType extends ReactProps {}
export function Card({ className = "", style = {}, ...props }: PropsType) {
  return (
    <div className={`p-4 bg-white rounded shadow ${className}`} style={{ ...style }}>
      {props.children}
    </div>
  );
}
      ...
      <div className="">
        <div className="flex justify-between ">
          ...
          <div className="flex flex-row-reverse items-center space-x-4 space-x-reverse">
            ...
            {props.actions && props.actions.map((btn, index) => <Button key={index} {...btn} />)}
          </div>
        </div>
      </div>
    </div>
