...
import { AgencyType, LuckyWheelModel, WheelStatus } from "../luckyWheel/luckyWheel.model";
import { CustomerModel } from "../customer/customer.model";
import { LuckyWheelHelper } from "../luckyWheel/luckyWheel.helper";
import { CustomerPointLogModel, CustomerPointLogType } from "../customerPointLog/customerPointLog.model";
...
const Mutation = {
    playLuckyWheel: async (root: any, args: any, context: Context) => {
        ...
        const [luckyWheel, customer, LuckyWheelGifts, luckyWheelResults] = await Promise.all([
            LuckyWheelModel.findOne({ _id: luckyWheelId, status: WheelStatus.OPEN }),
        ...
        if (!luckyWheel)
            throw ErrorHelper.mgRecoredNotFound('Vòng quay');

        // console.log('customer', customer);

        // kiem tra vòng quay có quà không
        if (LuckyWheelGifts.length === 0)
            throw ErrorHelper.mgRecoredNotFound('Quà của vòng quay.');

        // kiem tra ngày bắt đầu đến chưa ?
        const diffstart = LuckyWheelHelper.diffDate(luckyWheel.startDate, new Date());
        if (diffstart <= 0)
            throw ErrorHelper.requestDataInvalid('. Chưa đến ngày bắt đầu.');

        // kiem tra kết thúc chưa?
        const diffend = LuckyWheelHelper.diffDate(luckyWheel.endDate, new Date());
        if (diffend > 0)
            throw ErrorHelper.requestDataInvalid('. Giải thưởng đã kết thúc');
        ...
        if (luckyWheel.gamePointRequired > 0) {
            ...
            if (gamePointRemain < 0)
                throw ErrorHelper.notEnoughtPoint();
