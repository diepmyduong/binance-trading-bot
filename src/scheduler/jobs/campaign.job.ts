...
import { CampaignLoader, CampaignModel, ICampaign } from "../../graphql/modules/campaign/campaign.model";
import { CampaignSocialResultModel, MessageReceivingStatus } from "../../graphql/modules/campaignSocialResult/campaignSocialResult.model";
...
import { IProduct, ProductModel } from "../../graphql/modules/product/product.model";
...
const recordMessenger = async () => {
  const campaignRecords = await CampaignSocialResultModel.find({ messageReceivingStatus: MessageReceivingStatus.PENDING }).limit(100);
  for (const record of campaignRecords) {
    ...
    const result: any = await sendAllMessages(member, campaign, record.affiliateLink);
    ...
    await CampaignSocialResultModel.findByIdAndUpdate(record.id, { $set: params })
  }
};

const sendAllMessages = async (member: IMember, campaign: ICampaign, affiliateLink: String) => {
...
const sendBodyMessage = async (chatbotKey: any, psids: any, affiliateLink: any, campaign: ICampaign) => {

  const LINK_AFFILIATE = affiliateLink;
  const HASH_TAGS = campaign.hashtags ? campaign.hashtags.map((tag: string) => `#${tag} `).join('') : '';
  const contentParams = {
    apiKey: chatbotKey,
    psids: psids,
...
const sendHeaderMessage = async (chatbotKey: any, psids: any) => {
  const message = await SettingHelper.load(SettingKey.CAMPAIGN_HEADER_MSG_FOR_SHOPPER);
